...
export default async function RestaurantPage({
  ...
  const formatBusinessHours = (businessHours: BusinessHours[]) => {
    ...
    return businessHours.map(
      (hour) =>
        `${days[hour.weekDay]}: ${hour.openingTime} - ${hour.closingTime}`
    )
  }

  // Agrupar produtos por categoria (tag)
  const productsByCategory = store.products.reduce(
    (acc, product) => {
      if (!acc[product.tag]) {
        acc[product.tag] = []
      }
  ...
  return (
    <div className='min-h-screen bg-base-200'>
          ...
          {Object.entries(productsByCategory).map(([category, products]) => (
            <div className='collapse collapse-open' key={category}>
              <div
                ...
                className='collapse-title font-semibold mb-8 scroll-mt-16'
              >
      ...
      <div className='fixed bottom-4 right-4 z-30'>
        <button type='button' className='btn btn-primary btn-circle shadow-lg'>
