    ...
    return (
        ...
        >
            ...
            <div className="card-body p-3 relative">
                {product.promoActive && (
                    <div className="badge badge-secondary badge-sm absolute -top-1 -right-1 z-10">Promoção</div>
                )}
                <h4 className="card-title text-base font-bold line-clamp-1">{product.name}</h4>
                <p className="text-xs text-base-content/70 line-clamp-2">{product.description}</p>
                <div className="flex justify-between items-center mt-auto">
                    {product.promoActive ? (
                        <div className="flex flex-col">
                            <span className="text-xs line-through text-base-content/50">{formatCurrency(product.price)}</span>
                            <span className="text-lg font-bold text-secondary">{formatCurrency(product.promotionalPrice)}</span>
                        </div>
                    ) : (
